...
void dfs(int pos, vector<int> &color, vector<vector<pair<int, int>>> &v,
         int nowcolor) {
  color[pos] = nowcolor;
  for (int i = 0; i < v[pos].size(); i++) {
    if (color[v[pos][i].first] == -1) {
      if (v[pos][i].second % 2 == 0)
        dfs(v[pos][i].first, color, v, nowcolor);
      else
        dfs(v[pos][i].first, color, v, 1 - nowcolor);
    }
  }
}
