...
配点 : 500 点
問題文 : N 人の選手がテニスの大会に参加します。彼らを選手 1、選手
2、\ldots、選手 N と呼びます。大会は総当たり戦で、合計 N(N-1)/2
試合が行われます。これらの試合の日程を、以下の条件をすべて満たすように決めることは可能でしょうか。可能である場合、必要な最小の日数も求めてください。各選手は一日に最大で一試合を行う。各選手
i (1 \leq i \leq N) は、選手 A_{i, 1}, A_{i, 2}, \ldots, A_{i, N-1}
とこの順に一度ずつ試合を行う。
制約3 \leq N \leq 10001 \leq A_{i, j} \leq NA_{i, j} \neq iA_{i, 1}, A_{i, 2},
\ldots, A_{i, N-1}
はすべて異なる。入力入力は以下の形式で標準入力から与えられる。NA_{1, 1} A_{1, 2}
\ldots A_{1, N-1}A_{2, 1} A_{2, 2} \ldots A_{2, N-1}:A_{N, 1} A_{N, 2} \ldots
A_{N,
N-1}出力条件をすべて満たすように全試合の日程を決めることが可能なら必要な最小の日数を、不可能なら
-1 を出力せよ。
// ソースコードの引用元 :
