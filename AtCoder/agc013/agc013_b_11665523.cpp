...
配点 : 500 点
問題文 : N 頂点 M 辺の、連結な単純無向グラフが与えられます。頂点には 1 から N
までの番号がついており、辺には 1 から M までの番号がついています。辺 i は、頂点
A_i と頂点 B_i を結んでいます。次の条件を満たすパスを 1
つ見つけて、出力してください。2 個以上の頂点を通る同じ頂点を 2
度以上通らないパスの少なくとも一方の端点と直接辺で結ばれている頂点は、必ずパスに含まれるただし、この問題の
制約の下で、このようなパスが必ず存在することが証明できます。また、あり得る答えのうちどれを出力しても構いません。
制約2 \leq N \leq 10^51 \leq M \leq 10^51 \leq A_i < B_i \leq
N与えられるグラフは連結かつ単純（どの 2 頂点を直接結ぶ辺も高々 1
つ）である入力入力は以下の形式で標準入力から与えられる。N MA_1 B_1A_2 B_2:A_M
B_M出力条件を満たすパスを 1 つ見つけて出力せよ。出力の 1
行目には、パスに含まれる頂点の個数を出力せよ。出力の 2
行目には、パスに含まれる頂点の番号を、パスを形成するような順序で、空白区切りにして出力せよ。
// ソースコードの引用元 :
