...
問題文 :
アリの高橋くんは凸多角形状の板の上にいます。高橋くんは向いている方向にまっすぐ歩いていきますが、どの方向を向いているかはわかりません。高橋くんは板の周囲にたどり着くと落ちてしまいます。高橋くんの位置と板を構成する凸多角形の頂点の位置が与えられるので、高橋くんが板から落ちるまでに歩く最短の距離を求めてください。位置は全て２次元座標で与えられます。入力入力は以下の形式で標準入力から与えられる。x
yNx_1 y_1x_2 y_2:x_N y_N1 行目には、高橋くんがいる位置の座標を表す整数 x, y
(-100 ≦ x ≦ 100, -100 ≦ y ≦ 100) が空白区切りで与えられる。2
行目には、板を構成する凸多角形の頂点数を表す整数 N (3 ≦ N ≦ 10) が与えられる。3
行目からの N 行には、板の頂点の座標を表す整数 x_i y_i (-100 ≦ x_i ≦ 100, -100 ≦
y_i ≦ 100)
が空白区切りで与えられる。ただし、板の頂点は反時計回りの順で与えられる。高橋くんのいる位置は周囲を含まない板の内部である。板は凸多角形であることが保証される。出力高橋くんが板から落ちるまでに歩く最短の距離を
1 行に出力せよ。出力は絶対誤差あるいは相対誤差の少なくとも片方が 10^{−6}
以下であれば許容される。
// ソースコードの引用元 :
https://atcoder.jp/contests/arc042/submissions/10744821
// 提出ID : 10744821
// 問題ID : arc042_b
// コンテストID : arc042
// ユーザID : xryuseix
...
struct Point {
  ...
  friend inline Point operator*(const Point &p, const double a) {
  ...
  // 内積
  friend inline double InnerProduct(const Point &p, const Point &q) {
    return p.x * q.x + p.y * q.y;
  }
  // 外積
  friend inline double OuterProduct(const Point &p, const Point &q) {
    return p.x * q.y - p.y * q.x;
  }

  // 二次元のノーム(ユークリッド距離)を計算
  friend inline double norm2(const Point &p) {
...
Line crosspoint(const Line &L, const Line &M) {
  ...
  for (int i = 0; i < l.line.size() - 1; i++) {
    for (int j = 0; j < m.line.size() - 1; j++) {
      double d =
          OuterProduct(m.line[j + 1] - m.line[j], l.line[i + 1] - l.line[i]);
      if (abs(d) < EPS)
        continue;
      res.line.push_back(l.line[i] +
                         (l.line[i + 1] - l.line[i]) *
                             OuterProduct(m.line[j + 1] - m.line[j],
                                          m.line[j + 1] - l.line[i]) /
                             d);
    }
  }
...
int main() {
  ...
  v.pb(v[0]);
  rep(i, n) { chmin(ans, distance(p, v[i], v[i + 1])); }
  printf("%7Lf\n", ans);
}
