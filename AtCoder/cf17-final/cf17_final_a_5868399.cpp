...
配点 : 300 点
問題文 : 文字列 S が与えられます。高橋君はこの文字列の好きな位置に好きなだけ文字
A を挿入することができます。S を AKIHABARA に変えることはできるでしょうか？
制約1 \leq |S| \leq 50S
は大文字アルファベットのみからなる。入力入力は以下の形式で標準入力から与えられる。S出力S
を AKIHABARA に変えることができる場合は YES、できない場合は NO を出力せよ。
// ソースコードの引用元 :
