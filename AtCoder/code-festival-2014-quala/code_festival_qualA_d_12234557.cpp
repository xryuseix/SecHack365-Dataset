...
問題文 : 高橋君の電卓は壊れてしまっています。K
種類より多くの種類の数字を打つと、電卓が壊れてしまいます。整数 A
が与えられます。高橋君は、この整数を入力したいです。ですが、普通にその整数を打とうとすると、電卓が壊れてしまうことがあるため、電卓が壊れない範囲で、出来るだけ整数
A との差の大きさが小さい整数を入力します。この時、高橋君が入力する整数と、整数 A
の差の大きさがいくつになるかを出力しなさい。入力入力は以下の形式で標準入力から与えられる。A
K1 行目には、入力したい整数 A (1 ≦ A ≦ 10^{15}) と、入力可能な数字の種類数 K (1
≦ K ≦ 10) が、スペース区切りで与えられる。部分点1 ≦ A ≦ 100,000
の全てのケースに正解すると、 30
点が与えられる。残りの全てのケースに正解すると、さらに 70
点が与えられる。出力高橋君が入力する整数と、整数 A の差の大きさの最小値を 1
