...
配点 : 300 点
問題文 : 高橋君は英小文字からなる文字列 S を持っています。高橋君は S を NIKKEI
型に分割する方法が何通りあるか気になっています。S を 6
つの空でない連続する文字列に分割する方法であって、次の条件を満たすものの個数を求めてください。S
を分割してできた文字列を前から順に S_1, S_2, S_3, S_4, S_5, S_6 としたとき、S_2
と S_6 が等しく、かつ S_3 と S_4 が等しい。
制約6 \leq |S| \leq 500S
は英小文字から成る入力入力は以下の形式で標準入力から与えられる。S出力S を NIKKEI
型に分割する方法の個数を出力せよ。
// ソースコードの引用元 :
