...
問題文 : 英小文字から成る文字列 S が与えられます。英小文字または . から成る長さ
1 以上 3 以下の文字列 T が S にマッチするとは、次の条件を満たすことを表します。T
の長さを K とする。 S の連続する K 文字であって、T の . を自由にそれぞれ英小文字
1 文字に置き換えることで一致するような部分が存在する。英小文字または .
から成る長さ 1 以上 3 以下の文字列全てのうち、S
にマッチするものの個数を求めてください。
制約1 \leq |S| \leq 100S
は英小文字から成る入力入力は以下の形式で標準入力から与えられる。S出力英小文字または
. から成る長さ 1 以上 3 以下の文字列全てのうち、S
にマッチするものの個数を出力せよ。
// ソースコードの引用元 :
