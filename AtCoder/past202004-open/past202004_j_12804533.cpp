...
問題文 : 英小文字 a ~ z と ( と ) からなる文字列 S が与えられます。ある文字列 a
と b に対して a+b を a と b をこの順に結合した文字列とします。また、ある文字列 a
に対して rev(a) を a
を反転させた文字列とします。以下の操作をこれ以上操作が行えなくなるまで繰り返し行ったときの、最終的な文字列
S を出力してください。( や ) を含まない文字列 a (空文字列でもよい) に対して (
+a+ ) のような部分文字列が S の中にあったときに、その部分文字列を a+rev(a)
に置き換えるただし、文字列 S
で正しい括弧の対応が取れていること、すなわち最終的な文字列 S に ( や )
は含まれないことが保証されています。また、この条件下で最終的な文字列は一意に定まることが示せます。
制約S は長さ 1 以上 1000 以下の、英小文字 a ~ z と ( と )
からなる文字列である。S は 1
文字以上の英小文字を含む。操作をこれ以上操作が行えなくなるまで繰り返し行ったときの、最終的な文字列
S の長さは 1000 以下であり、これに( や )
は含まれない。入力入力は以下の形式で標準入力から与えられる。S出力操作をこれ以上操作が行えなくなるまで繰り返し行ったときの最終的な文字列
S を出力せよ。
// ソースコードの引用元 :
