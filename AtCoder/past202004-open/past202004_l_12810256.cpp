...
問題文 : 長さ N の整数列 A_1, A_2, \cdots, A_N があります。ここから K
個の要素を選び、それらを相対的な順序を保ったまま並べて新しい数列を作るという操作を考えます。ただし、A_i
と A_j (i \neq j) をともに選べるのは |i - j| \geq D
であるときに限ります。この操作で作ることのできる数列のうち、辞書順で最小である数列を求めてください。そのような操作が不可能である場合は
-1 を出力してください。なお、長さ K の数列 x_1,x_2,\cdots,x_K が
y_1,y_2,\cdots,y_K より辞書順で小さいとは、二つの数列が異なり、かつ i を x_i
\neq y_i であるような最小の整数としたとき x_i < y_i であることをいいます。
制約2 \leq N \leq 2 \times 10^51 \leq D \leq N1 \leq K \leq N0 \leq A_i \leq
10^9入力は全て整数入力入力は以下の形式で標準入力から与えられる。N K DA_1 A_2
\cdots A_N出力操作が可能であれば得られる辞書順最小の数列を、不可能であれば -1
を出力せよ。
// ソースコードの引用元 :
